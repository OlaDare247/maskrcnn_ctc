...
def refine_instance_mask(label_mask, min_size=10, min_distance=5):
    binary = label_mask > 0

    if binary.sum() == 0:
        return label_mask.astype(np.uint16)

    # Distance map for splitting touching cells
    distance = ndi.distance_transform_edt(binary)

    # Find likely cell centers
    ...
    # If no paks are found, fall back to connected components
    ...
    # Watershed splitting
    refined = watershed(-distance, markers, mask=binary)

    # Remove tiny components and relabel cleanly
